...
public class CountryService
        implements IClassifierService<Country, CountryReadDTO, CountryCreateDTO> {
    ...
    @Override
    public CountryReadDTO add(CountryCreateDTO dto) {
    ...
    public CountryReadDTO findByUUID(UUID uuid) {
        return this.repository.findById(uuid)
                .map(mapper::toDTO)
                .orElseThrow(() -> {
                    throw new IllegalArgumentException(errorMessage.getCountryNotFound());
                });
    }
}
